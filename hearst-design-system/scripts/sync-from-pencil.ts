...
function main() {
  ...
  // Generate tokens.css
  const tokensCss = generateTokensCss(brands, globalVars, themedVars);
...
// Production headline fonts per brand (from CDN @font-face analysis).
// These override the Pencil font-family-default because production sites
// use specific display fonts for headlines that differ from the body font.
const BRAND_HEADLINES: Record<string, [string, number]> = {
  ...
  "elle": ["Modern MT Pro", 400],
  "harpers-bazaar": ["NewParis Text", 400],
  "good-housekeeping": ["Barlow Semi Condensed", 800],
  "house-beautiful": ["SangBleu Sunrise", 500],
  "country-living": ["Playfair Display", 400],
  "delish": ["TT Commons Pro", 700],
  "mens-health": ["Manrope", 800],
  "womens-health": ["Altone", 700],
  "town-and-country": ["Montserrat", 600],
  "veranda": ["Playfair Display", 400],
  "elle-decor": ["Modern MT Pro", 400],
  "road-and-track": ["Buzz", 900],
  "runners-world": ["League Gothic", 400],
  "bicycling": ["Velo Serif Display", 700],
  "popular-mechanics": ["United Sans Cd", 400],
  "prevention": ["Poppins", 700],
  "best-products": ["Inter", 700],
  "oprah-daily": ["Juana", 500],
  "seventeen": ["Inter", 800],
  "marie-claire": ["Modern MT Pro", 400],
  "autoweek": ["Inter", 700],
  "biography": ["Playfair Display", 400],
  "food-network": ["Inter", 700],
  "hgtv": ["Inter", 700],
  "womans-day": ["League Spartan", 600],
};

// Production secondary font overrides (eyebrows, labels, subheads).
const SECONDARY_OVERRIDES: Record<string, string> = {
  "car-and-driver": "Barlow Semi Condensed",
  "cosmopolitan": "Chronicle Display",
  "esquire": "Lausanne",
  "elle": "Modern MT Pro",
  "harpers-bazaar": "NewParis Text",
  "good-housekeeping": "Barlow Semi Condensed",
  "house-beautiful": "Apparel",
  "country-living": "Playfair Display",
  "town-and-country": "Montserrat",
  "veranda": "Playfair Display",
  "elle-decor": "Modern MT Pro",
  "oprah-daily": "Juana",
  "road-and-track": "Roboto",
  "runners-world": "Futura Now Text",
  "popular-mechanics": "Work Sans",
  "womans-day": "Arvo",
};
...
function generateBrandsTs(brands: BrandData[]): string {
  const lines: string[] = [
    ...
    "// NOTE: fontHeadline, fontHeadlineWeight, and fontSecondary overrides",
    "// are maintained in the sync script (BRAND_HEADLINES, SECONDARY_OVERRIDES).",
    "",
  ...
  for (const b of brands) {
    ...
    const sec = SECONDARY_OVERRIDES[b.slug];
    const fontSecondary = sec || b.fontSecondary;
    const fontHeadline = hl ? hl[0] : b.fontDefault;
    const fontHeadlineWeight = hl ? hl[1] : 700;
    ...
    lines.push(`    fontHeadline: ${JSON.stringify(fontHeadline)},`);
    lines.push(`    fontHeadlineWeight: ${fontHeadlineWeight},`);
...
function generateTokensCss(
  ...
  for (const [name, def] of Object.entries(globalVars).sort(([a], [b]) =>
    ...
    lines.push(`  --${fixTypos(name)}: ${cssValue};`);
  }
  lines.push("}");
  lines.push("");
  ...
  for (const brand of brands) {
    ...
    // Convenience aliases that map Pencil names to what ThemeProvider/components expect.
    // --brand-primary aliases palette-brand-1 for backward compatibility.
