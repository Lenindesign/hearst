...
Reader profile, preference, and stakeholder-control state is browser-local demo data. First-use personalization asks for one or two interests and previews matching real stories before applying the preference; it does not require or imply an account. For You river cards expose one quiet, source-backed explanation for their ranking without showing the stakeholder scoring model. High-value For You modules use a deterministic inventory allocation so Today&rsquo;s Picks, Today&rsquo;s Edit, Daily Habit, Trending Across Brands, and the river have distinct stories while small catalogs retain a usable feed. Stakeholder controls and technical explanation panels are hidden from the default reader experience and require explicit demo mode. A typed, browser-only product-event contract supports prototype measurement without sending data to a vendor. Production identity, cross-device sync, consent, analytics collection, dashboards, CMS publishing, and experimentation are not completed integrations. The RSS catalog changes only after the import, byline-enrichment, and validation workflow succeeds; production delivery requires that workflow to be scheduled and monitored before each daily morning edition.
